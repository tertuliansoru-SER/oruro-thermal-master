...
window.iniciarTurno = function() {
    ...
    state.turnoActual = { nombre: nombre.toUpperCase(), hora: hora, punta: punta, inicio: Date.now() };
...
window.guardarRegistro = async function() {
    if (!state.isOnline) { alert('Sin conexión. Intente nuevamente.'); return; }
    ...
    setTimeout(() => { btn.textContent = 'Guardar Registro'; btn.classList.remove('bg-green-600'); }, 1500);
};
...
window.descargarCSV = function() {
    if (state.logs.length === 0) { alert('No hay datos para exportar'); return; }
